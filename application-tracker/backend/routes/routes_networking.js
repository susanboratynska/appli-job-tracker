...
appliRoutes.route('/networking/add').post(function (req, res) {
    ...
    networking.save()
        .then(networking => {
            res.status(200).json({ 'Networking': 'networking event added successully' });
        })
        .catch(err => {
...
appliRoutes.route('/networking/update/:id').post(function (req, res) {
    NetworkingEvent.findById(req.params.id, function (err, networking) {
        ...
        networking.save()
            .then(networking => {
                res.json('Networking Event updated')
            })
            .catch(err => {
...
appliRoutes.route('/networking/getPublicEvents').get((req, res) => {
...
//get all public events
appliRoutes.route('/networking/getPublicEventsShort').get((req, res) => {
...
appliRoutes.route('/networking/getMyEvents/:id').get((req, res) => {
    ...
    NetworkingEvent.find({ 'creatorID': req.params.id }).sort({event_date: 1})
        .then((event) => {
            res.json(event);
        })
        .catch(err => res.status(400).json('Error: ' + err));
...
appliRoutes.route('/networking/getMyEventsShort/:id').get((req, res) => {
    console.log("============");
    console.log("Sending short list (limit 3) of all Networking Events created by the user with id = " + req.params.id + " to the front end...");
    ...
    NetworkingEvent.find().sort({event_date: 1}).limit(3)
        .then((event) => {
            res.json(event);
        })
        .catch(err => res.status(400).json('Error: ' + err));
